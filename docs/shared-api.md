...
성공한 검색 결과는 최초 `fetchedAt`부터 5분간 재사용합니다. 브라우저의 공용 검색 캐시는 서버 주소·닉네임별로 최대 5개를 현재 탭의 `sessionStorage`에 저장하고, 저장소를 사용할 수 없으면 메모리에서 동작합니다. 개인 키 검색은 키별로 분리한 메모리 캐시만 사용하며 키를 저장소에 기록하지 않습니다. 서버는 실행 중인 인스턴스마다 정규화한 결과 최대 100개를 보관합니다. 서버 재시작이나 실행 인스턴스가 달라지면 새로 조회할 수 있습니다.

**최신 정보 다시 불러오기**는 완료된 브라우저·서버 캐시를 우회합니다. 동일한 조회가 진행 중이면 요청을 공유하며, 한 사용자의 취소가 다른 사용자의 조회를 취소하지 않습니다. 모든 대기자가 취소하면 upstream 요청도 중단합니다. 오류·시간 초과·취소 결과는 캐시하지 않으며, 캐시를 재사용해도 원래 조회 시각은 바뀌지 않습니다. 요청 제한과 허용 출처 확인은 서버 캐시보다 먼저 수행하고, 응답 헤더는 요청별로 생성합니다.
...
- `GET /api/character?name=깽미니&refresh=1` → 완료된 캐시를 우회한 `CharacterSnapshot`.
- `GET /share?character=깽미니&mode=cube` → 닉네임 공유 미리보기 HTML. `HEAD`도 지원합니다.
- 캐시가 없거나 만료되었을 때 ID, 기본 정보, 장비, 어빌리티의 고정된 Nexon API 4개를 조회합니다. 다른 URL을 지정하는 범용 프록시가 아닙니다.
- 응답은 계속 `no-store`이며 캐시는 검증된 스냅샷에만 적용합니다. CORS 헤더·오류 응답·키·OCID는 캐시에 포함하지 않으며 실제 `fetchedAt`을 보존합니다.
